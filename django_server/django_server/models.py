...
class Person(Base1):
    ...
    blog = models.ForeignKey(Base2, on_delete=models.CASCADE) # tạo Foreign key giữa Person và Base2
    ShirtSize = models.TextChoices("ShirtSize", "Small Medium Large") #other way to define choice
    shirt_size = models.CharField(max_length=1, choices=SHIRT_SIZES)
    m2m = models.ManyToManyField(Base1) # 1 query set chứa nhiều đối tượng Base1, các object Base1 cũng có thể truy cập được đến các đối tượng chứa nó (A QuerySet represents a collection of objects from your database, tương đương vs lệnh SELECT trong SQL)
...
f=Person.objects.exclude(pub_date__lte="2006-01-01") # pub_date<=2006-01-01 #gt,lt,gte,...
f=Person.objects.order_by("headline")

# FILTER ĐÔI
Person.objects.filter(blog__headline__contains="Lennon", blog__pub_date__year=2008) #blog thỏa mãn cả 2 đk
Person.objects.filter(blog__headline__contains="Lennon").filter(
    blog__pub_date__year=2008 ) #blog thỏa mãn 1 điều kiện
...
print([p.headline for p in queryset])  # Evaluate the query set. (cached) (phải duyệt qua toàn bộ)